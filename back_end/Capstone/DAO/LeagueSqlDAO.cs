...
namespace Capstone.DAO
{
    public class LeagueSqlDAO : ILeagueDAO
    {
        ...
        {
            League returnLeague = new League();
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    SqlCommand cmd = new SqlCommand("select * from league join league_users on league.league_id = league_users.league_id join users on users.user_id = league_users.user_id where league.league_id = @leagueId", conn);
